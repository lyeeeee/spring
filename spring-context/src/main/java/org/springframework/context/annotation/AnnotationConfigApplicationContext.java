...
public class AnnotationConfigApplicationContext extends GenericApplicationContext implements AnnotationConfigRegistry {
	...
	public AnnotationConfigApplicationContext() {
		...
		//定义了一个扫描器,扫描包或者类，继而转化为bd
		//但实际上我们扫描包不是scanner这个对象
		//是spring自己new的一个ClasspathBeanDefinationScanner
		//这里的scanner仅仅是为了程序员能够在外部调用context对象的scan方法
	 ...
	 * 注册单个bean容器
	 * 比如有新加的类可以用这个方法
	 * 但是注册了之后要手动调用refresh方法去触发容器解析注解
	 *
	 * 有两个意思，第一个可以注册一个配置类，还可以单独注册一个bean
	 *
