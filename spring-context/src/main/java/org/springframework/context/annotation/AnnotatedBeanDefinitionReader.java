...
public class AnnotatedBeanDefinitionReader {
	 ...
	 * @since 5.0
	 */
	// 传入的beanClass一定带注解，可以理解为config类也是一种bean
	// 注解 可以注解在bean上，也可以直接通过接口传进来
	<T> void doRegisterBean(Class<T> beanClass, @Nullable Supplier<T> instanceSupplier, @Nullable String name,
			@Nullable Class<? extends Annotation>[] qualifiers, BeanDefinitionCustomizer... definitionCustomizers) {

		// 根据指定的bean创建一个AnnotatedGenericBeanDefinition
		// 这个AnnotatedGenericBeanDefinition可以理解为一个数据结构,是基于注解的bean定义
		// 包含了类的其他信息，比如一些元信息scope lazy等
		AnnotatedGenericBeanDefinition abd = new AnnotatedGenericBeanDefinition(beanClass);

		// 判断是否要跳过解析,没有元数据，就不需要解析它
		...
		// 生成bean的名字，通过beanNameGenerator
		String beanName = (name != null ? name : this.beanNameGenerator.generateBeanName(abd, this.registry));

		// 处理类当中通用的注解，例如lazy dependOn primarily role
		...
		// beanDefinitionHolder 持有一个definitionHolder
		BeanDefinitionHolder definitionHolder = new BeanDefinitionHolder(abd, beanName);
		/**
		 * scopedProxyMode需要接和springmvc理解
		 * */
		definitionHolder = AnnotationConfigUtils.applyScopedProxyMode(scopeMetadata, definitionHolder, this.registry);
		/**
		 * 把上述数据结构注册给registry
		 * registry就是AnnotationConfigApplicationContext
		 * AnnotationConfigApplicationContext在初始化的时候曾经调用过一个父类构造方法
		 * 曾实例化了一个DefaultListenableBeanFactory
		 *
		 * registerBeanDefinition这里的意思就是把definitionHolder这个数据结构包含的信息注册到DefaultListenableBeanFactory这个工厂
		 * */
